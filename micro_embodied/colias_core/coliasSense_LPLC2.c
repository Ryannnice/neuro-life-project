...
void LPLC2_Init(LPLC2_pControlTypedef* hLPLC2)
{
    ...
    LPLC2.Params.Nc = 2 ; // connected neuron number
...
*/
    ...
    // Visuomotor Initialization :
    LPLC2.Visuomotor.Threat_Strength = 0 ;
    LPLC2.Visuomotor.Threat_Direction = 0 ;
    LPLC2.Visuomotor.Escape_Direction = 0 ;
    LPLC2.Visuomotor.Escape_Speed = 0 ;
...
LPLC2.TIME[0] = TOCin5 ;
        ...
        } // We've Got OFF Layer after Filtering. // 1.7ms
LPLC2.TIME[1] = TOCin5 ;
...
LPLC2.TIME[3] = TOCin5 ;
        ...
        for(i = 0; i < NUM_AF; i ++ )
        {
            ...
            if(AF[i].Vacant_AF == 0 ){
                // Reset AF:
                d = AF[i].Centroid[0] + AF_Radius_Exclusive ; // AF_Radius ; // This is wrong since it only update the inner AF status below(line 524 ).
                u = AF[i].Centroid[0] - AF_Radius_Exclusive ; // AF_Radius ; // This is wrong since it only update the inner AF status below(line 524 ).
                r = AF[i].Centroid[1] + AF_Radius_Exclusive ; // AF_Radius ; // This is wrong since it only update the inner AF status below(line 524 ).
                l = AF[i].Centroid[1] - AF_Radius_Exclusive ; // AF_Radius ; // This is wrong since it only update the inner AF status below(line 524 ).
                for(row = u; row < d; row ++ )
                {
                    if(row < 0 || row >= Height )
                        continue ; 
        ...
        } // Traversal AFs Over ... // 0.3 ms
LPLC2.TIME[4] = TOCin5 ; // 5.6 ms
...
} // Pre-synaptic Calculation Over ... // < 6 ms
...
void LPLC2_T4_T5(LPLC2_pControlTypedef* hLPLC2)
{
    ...
    uint8_t d = Params->d ;
    uint8_t n = Params->Nc ;
    uint8_t delay = Params->Delay ; // Delay = TIME_WINDOW - 1
...
*/
    ...
    for(N = 0; N < NUM_AF; N ++ ) 
    {
        ...
        for (y = AF_y_top + n * d ; y < AF_y ; y += step ) // n=3, d=1, step=1
        {
            for (x = AF_x ; x < AF_x_right - n * d ; x += step )
            {
                //Layers->T[y][x] = 0 ;
                for(i = 1; i <= n; i ++ )
                {
                    if((y-i*d<0)||(y+i*d>=Height)||(x-i*d<0)||(x+i*d>=Width) )
                        continue ; 
                    /*
                    Right = (Layers->ON [cur][y][x + i*d] * Layers->ON [pre][y][x])
                          - (Layers->ON [pre][y][x + i*d] * Layers->ON [cur][y][x]) // >> bias; // PROBLEM CORE
...
*/
        ...
        for(n = 0; n < NUM_AF; n ++ )
        {
            ...
            // LPLC2 Projection :
            if(expansion_edge_count < Params->T_ExpansionEdgeNum /*Q[0] * Q[1] * Q[2] * Q[3] == 0*/ ){
        ...
        for(n = 0; n < NUM_AF; n ++ )
        {
            ...
            // It Costs 5ms to Excute Single One "printf();" in Colias !!!!!!!!!!!!
            ...
            // // 
            // // 
            // printf("\n %lld  Current LPLC2 Output, cur LPLC2: %d ", AF[n].Output[(*(hLPLC2->hFrameCount)-1)%4], (*(hLPLC2->hFrameCount)-1)%4 ) ;
            // printf("\n %lld  Recent LPLC2 Output ", AF[n].LPLC2 ) ;
            // //    printf("\n %f  I ", AF[N].I ) ;
            // //    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/* 
// LPLC2 'X' cross shape, call HRC() function version :

//This version call founciton HRC(), which cause redundant calculation.

// Quadrant 1 : | |1| upper right
//              | | |
for(y = 0 ; y < AF_y - n*d ; y += step )
{
    for(x = AF_x - n*d ; x < Width ; x += step )
    {
        Right = HRC(Layers->ON[cur][y][x], Layers->ON[cur][y][x-step],
                    Layers->ON[pre][y][x], Layers->ON[pre][y][x-step], bias) // ON
              + HRC(Layers->OFF[cur][y][x], Layers->OFF[cur][y][x-step],
                    Layers->OFF[pre][y][x], Layers->OFF[pre][y][x-step], bias) ; // OFF
        Up = HRC(Layers->ON[cur][y][x], Layers->ON[cur][y+step][x],
                 Layers->ON[pre][y][x], Layers->ON[pre][y+step][x], bias) // ON
           + HRC(Layers->OFF[cur][y][x], Layers->OFF[cur][y+step][x],
                 Layers->OFF[pre][y][x], Layers->OFF[pre][y+step][x], bias) ; // OFF
        // Pixel (y,x) in quadrant 1 :
        Layers->T[y][x] = (uint32_t)sqrt(Right*Right + Up*Up) ;
        Results->Quadrant[0] += Layers->T[y][x] ;
    }
}
...
for(y = 0 ; y < AF_y - n*d ; y += step )
{
    for(x = 0 ; x < AF_x - n*d ; x += step )
    {
        Left = HRC(Layers->ON[cur][y][x], Layers->ON[cur][y][x+step], 
                   Layers->ON[pre][y][x], Layers->ON[pre][y][x+step], bias) // ON
             + HRC(Layers->OFF[cur][y][x], Layers->OFF[cur][y][x+step], 
                   Layers->OFF[pre][y][x], Layers->OFF[pre][y][x+step], bias) ; // OFF
        Up = HRC(Layers->ON[cur][y][x], Layers->ON[cur][y+step][x],
...
// Quadrant 3 :             | | |
//              bottom left |3| |
for(y = AF_y + n*d ; y < Height ; y += step )
{
    for(x = 0 ; x < AF_x - n*d ; x += step )
    {
        Left = HRC(Layers->ON[cur][y][x], Layers->ON[cur][y][x+step],
             ...
             + HRC(Layers->OFF[cur][y][x], Layers->OFF[cur][y][x+step],
                   Layers->OFF[pre][y][x], Layers->OFF[pre][y][x+step], bias) ; // OFF
        Down = HRC(Layers->ON[cur][y][x], Layers->ON[cur][y-step][x],
                   Layers->ON[pre][y][x], Layers->ON[pre][y-step][x], bias) // ON
             + HRC(Layers->OFF[cur][y][x], Layers->OFF[cur][y-step][x], 
                   Layers->OFF[pre][y][x], Layers->OFF[pre][y-step][x], bias) ; // OFF
        // Pixel (y,x) in quadrant 3 :
        Layers->T[y][x] = (uint32_t)sqrt(Left*Left + Down*Down) ;
        Results->Quadrant[2] += Layers->T[y][x] ;
    }
}
